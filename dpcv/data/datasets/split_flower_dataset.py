...
def move_img(imgs, root_dir, set_dir_name):
    data_dir = os.path.join(root_dir, set_dir_name)
    my_mkdir(data_dir)
    for path_img in imgs:
        print(path_img)
        shutil.copy(path_img, data_dir)
    print("{} dataset, copy {} imgs to {}".format(set_dir_name, len(imgs), data_dir))
...
if __name__ == '__main__':
    ...
    test_ratio = 0.1
    # 1. 读取list，打乱
    root_dir = r"/datasets/102flowers"
...
r"""
train dataset, copy 6551 imgs to path\to\102flowers\train
valid dataset, copy 819 imgs to path\to\102flowers\valid
test dataset, copy 819 imgs to path\to\102flowers\test
"""
